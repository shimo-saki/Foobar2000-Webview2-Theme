...
> **v2.5.0** — 多编码歌词引擎 · GBK/Big5/Shift_JIS 自动识别 · 歌词解析重构
...
- **动态歌词**: 集成歌词显示功能，支持逐字高亮、双语合并和独立的全屏歌词模式。内置编码检测引擎，基本可正确识别 GBK/UTF-8/UTF-16 编码的 LRC 文件。
...
- `js/`: 核心业务逻辑与渲染模块。
    - `core.js`: API 封装与状态管理。
    - `lyric-core.js`: 多编码歌词引擎（编码检测 + 模块化解析器）。
    - `ui.js`: 基础渲染层。
