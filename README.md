...
> **v2.5.2** — 双语配对自动归一 · 歌词面板右键菜单 · 副标题行归位
...
- **双语配对自动归一**：歌词的两种常见写法（同刻：原文与译文同一时间戳；译文延后一行：译文写在下一句的时间上）现在会被自动识别并统一成“原文为主、译文为副”的正序显示。判据按脚本族分侧（中日/中韩/英日/俄中等任意两种文字均成立）、不依赖语料分布；判不出的文件可在歌词面板右键菜单里手动指定，选择按文件记忆。
...
- **歌词面板右键菜单**：在歌词面板或沉浸式页面的歌词区右键 ——「翻译对齐方式」是一个二级菜单（鼠标悬停展开，当前项带 ✓）：**自动识别 / 标准双语 / 兼容旧版**，选择按文件记忆；下方是 刷新歌词 / 复制歌词 / 打开所在文件夹。歌词没有同刻行时（纯文本、逐字歌词等）不显示对齐项。
- **其它修复**：歌词为空时右键菜单不再残留上一首的识别结果；二级菜单在主菜单滚动时自动收起；归一后组时间戳可能倒退（影响高亮二分查找的正确性）、localStorage 损坏时的崩溃。
- **伪代码清理**：移除判定路上重复的脚本族分类与最坏 O(n²) 关系式扫描，单文件分析耗时在最坏情况下减半。
