...
- **导出**：选「标准双语」（译文与原文同刻）或「兼容旧版」（译文写在下一句原文时间戳上）后下载；**只改时间戳与行顺序，不改一个字**；元信息行（`[ti:]` `[ar:]` …）原样保留；UTF-8 带 BOM，可直接覆盖原文件（建议先备份）
...
**双语歌词编辑器（可选工具，独立于主题）**
- `tools/lrc-normalizer.html`**不放在 default 里、主题不加载它**：浏览器直接打开即可，纯本地运行。一键识别打底，再手动调整 —— 点角色徽标切 主/译文、↑ ↓ 移动、＋ 插入/追加行、✕ 删除、时间戳与正文都**就地编辑**（回车提交 / Esc 取消）、Ctrl+Z/Y 撤销重做、整窗口拖放换文件、清空按钮；右侧实时预览主题里的显示效果；导出两种写法（标准双语 / 兼容旧版）。
