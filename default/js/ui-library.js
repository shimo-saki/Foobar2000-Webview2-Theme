...
(function () {
  ...
  CM.renderLibrary = function () {
    ...
    _libTreeNodes.forEach(el => el.classList.toggle('active', el.dataset.view === active));

    LIB_RENDERERS[state.libraryView]?.();
  };
  ...
  CM._pagerBarHtml = function (prefix, title, total) {
    const pageSize = CM._libPageSize();
